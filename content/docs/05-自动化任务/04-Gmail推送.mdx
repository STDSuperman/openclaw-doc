...
tags: ["gmail", "pubsub", "邮件通知", "推送"]
prerequisites: ["自动化任务概览", "Webhooks"]
related_docs: ["Webhooks", "自动化任务概览"]
next_steps: []
...
- OpenClaw hooks 已启用(参见 [Webhooks](/05-自动化任务/03-Webhooks))
- `tailscale` 已登录([tailscale.com](https://tailscale.com/))。支持的设置使用 Tailscale Funnel 获取公共 HTTPS 端点
  其他隧道服务也可以工作,但是 DIY/不支持的,需要手动接线
  目前,Tailscale 是我们支持的
...
要将 Gmail 摘要投递到聊天表面,使用设置了 `deliver` + 可选 `channel`/`to` 的映射覆盖预设:
    ...
    mappings: [
      {
        ...
        messageTemplate: "来自 {{messages[0].from}} 的新邮件\n主题: {{messages[0].subject}}\n{{messages[0].snippet}}\n{{messages[0].body}}",
...
如果你需要固定频道,设置 `channel` + `to`。否则 `channel: "last"`使用最后投递路由(回退到 WhatsApp)。

要强制 Gmail 运行使用更便宜的模型,在映射中设置 `model`(`提供商/模型`或别名)。如果你强制 `agents.defaults.models`,请包含它。

要专门为 Gmail hooks 设置默认模型和思考级别,在你的配置中添加 `hooks.gmail.model` / `hooks.gmail.thinking`:
...
要进一步自定义负载处理,在 `hooks.transformsDir` 下添加 `hooks.mappings` 或 JS/TS 转换模块(参见 [Webhooks](/05-自动化任务/03-Webhooks))

## 向导(推荐)

使用 OpenClaw 帮助器将所有内容接线在一起(在 macOS 上通过 brew 安装依赖):
...
平台注意:在 macOS 上向导通过 Homebrew 安装 `gcloud`, `gogcli` 和 `tailscale`;在 Linux 上手动安装它们。

Gateway 自动启动(推荐):

- 当 `hooks.enabled=true` 并设置了 `hooks.gmail.account` 时,Gateway 启动
  时在引导时运行 `gog gmail watch serve` 并自动续期 watch
- 设置 `OPENCLAW_SKIP_GMAIL_WATCHER=1` 选择退出(如果你自己运行守护进程,这很有用)
- 不要同时运行手动守护进程,否则你会遇到
...
选择 **拥有** `gog` 使用的 OAuth 客户端的 GCP 项目
...
推荐:`openclaw webhooks gmail run` 包装相同的流程并自动续期 watch。

## 暴露处理程序(高级,不支持)

如果你需要非 Tailscale 隧道,手动接线并在推送订阅中使用公共 URL(不支持,没有护栏):
