...
<ai-only>
**AI 指令区**：本文档说明如何通过 CLI 和编程接口发送消息，包括基本用法、参数选项和高级场景。
</ai-only>
...
| 参数 | 必需/可选 | 说明 | 示例 |
|------|------------|------|------|
| `--target` | 必需 | 目标接收方（根据通道类型） | `+86138000000`、`@username`、`#channel` |
| `--message` | 必需 | 消息内容 | `"Hello World!"` |
| `--channel` | 可选 | 指定消息通道 | `whatsapp`、`telegram`、`discord` |
| `--session` | 可选 | 指定会话（默认使用路由规则） | `agent:main:whatsapp:dm:+86138000000` |
...
**示例会话**：
- `agent:main:whatsapp:dm:+86138000000` - 主会话，私聊
...
```bash
# 每天早上 9 点发送问候
echo "0 9 * * * $(date +\%m) $(date +\%d) openclaw message send --target +86138000000 --message '早上好！今天是 $(date +\%m)月$(date +\%d)日，有什么可以帮你的吗？'" | crontab -
```
